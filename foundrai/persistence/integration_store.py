...
class IntegrationStore:
    ...
    async def delete_integration(self, integration_id: str) -> bool:
        ...
        cursor = await self.db.conn.execute("DELETE FROM integrations WHERE id = ?", (integration_id,))
        await self.db.conn.commit()
    ...
    async def delete_task_mapping(self, mapping_id: str) -> bool:
        ...
        cursor = await self.db.conn.execute("DELETE FROM external_task_mappings WHERE id = ?", (mapping_id,))
        await self.db.conn.commit()
        return cursor.rowcount > 0
    
    def _row_to_integration(self, row) -> IntegrationConfig:
        """Convert database row to IntegrationConfig object."""
        from foundrai.models.integration import IntegrationStatus, IntegrationType
    ...
    def _row_to_task_mapping(self, row) -> ExternalTaskMapping:
        ...
        return ExternalTaskMapping(
            ...
            external_url=row[4],
            last_sync=datetime.fromisoformat(row[5]) if row[5] and isinstance(row[5], str) else row[5],
