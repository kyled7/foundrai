...
class TestTemplateStore:
    ...
    async def test_create_template(
        self, template_store: TemplateStore, sample_template: TeamTemplate
    ):
...
class TestTemplateManager:
    ...
    def sample_config(self) -> FoundrAIConfig:
        ...
        return FoundrAIConfig(
            team=TeamConfig(), sprint=SprintConfig(max_tasks_parallel=2, token_budget=50000)
        )

    async def test_save_template(
        self, template_manager: TemplateManager, sample_config: FoundrAIConfig
    ):
    ...
    async def test_load_template(
        self, template_manager: TemplateManager, sample_config: FoundrAIConfig
    ):
        """Test loading a template."""
        # Save template first
        template = await template_manager.save_template(
            name="load-test", config=sample_config, author="test-author"
        )
    ...
    async def test_apply_template(
        ...
        custom_sprint_config = SprintConfig(
            max_tasks_parallel=5, token_budget=100000, max_sprints=3
        )
