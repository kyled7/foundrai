...
class TestTeamStore:
    ...
    def sample_team(self) -> Team:
        ...
        return Team(
            ...
            agents=[
                AgentConfig(
                    enabled=True, model="anthropic/claude-sonnet-4-20250514", autonomy="notify"
                ).model_dump()
            ],
            template_id=None,
...
class TestCrossTeamDependencies:
    ...
    async def test_create_dependency(
        self, team_store: TeamStore, sample_dependency: CrossTeamDependency
    ):
    ...
    async def test_resolve_dependency(
        self, team_store: TeamStore, sample_dependency: CrossTeamDependency
    ):
    ...
    async def test_dependency_status_progression(
        self, team_store: TeamStore, sample_dependency: CrossTeamDependency
    ):
...
class TestMultiTeamCoordination:
    ...
    async def test_dependency_resolution_workflow(self, team_store: TeamStore):
        ...
        # Create teams
        frontend = Team(
            name="Frontend", project_id="app", agents=[], sprint_config=SprintConfig().model_dump()
        )
        backend = Team(
            name="Backend", project_id="app", agents=[], sprint_config=SprintConfig().model_dump()
        )
    ...
    async def test_multiple_dependencies_same_teams(self, team_store: TeamStore):
        ...
        # Create teams
        mobile = Team(
            name="Mobile", project_id="app", agents=[], sprint_config=SprintConfig().model_dump()
        )
        backend = Team(
            name="Backend", project_id="app", agents=[], sprint_config=SprintConfig().model_dump()
        )
